"""
=============================================================================
Friend Recommendation System using Link Prediction (GAT — Graph Attention Network)
=============================================================================

This variant replaces PyG's GCNConv with **GATConv** (Graph Attention
Networks — Veličković et al., 2018).  Instead of fixed neighbourhood
aggregation weights (as in GCN), GAT learns to *attend* to the most
relevant neighbours using multi-head attention.

Architecture
────────────
    Layer 1: GATConv(in → hidden, heads=4, concat=True)
             → Output dim = hidden × heads
    Layer 2: GATConv(hidden × heads → out, heads=1, concat=False)
             → Output dim = out

All other pipeline components (feature engineering, edge splitting,
decoding, evaluation, and visualisations) remain identical.
...
from torch_geometric.nn import GATConv                      # ← Changed
...
# ══════════════════════════════════════════════════════════════════════════
# 3.  GAT ENCODER  (Graph Attention Network)
# ══════════════════════════════════════════════════════════════════════════
class GATEncoder(nn.Module):
    """
    A 2-layer Graph Attention Network with multi-head attention.

    Layer 1: GATConv with `heads` attention heads (outputs are concatenated)
             → effective output dim = hidden_channels × heads
    Layer 2: GATConv with 1 head (no concatenation)
             → output dim = out_channels

    The attention mechanism learns to weight different neighbours
    differently, allowing the model to focus on the most informative
    ...
    def __init__(self, in_channels: int, hidden_channels: int = 128,
        ...
        # Single-head attention layer 2
        # Input = hidden_channels * heads (from concatenation)
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        ...
        # Layer 1: multi-head attention + ELU activation
...
def dot_product_decode(z, edge_label_index):
    """Dot-product link scoring — used by evaluate() for AUC/AP."""
...
def train_one_epoch(model, optimizer, train_data):
    ...
    Steps
    ─────
    1. Compute node embeddings via the GAT encoder.
...
# ══════════════════════════════════════════════════════════════════════════
def plot_learning_curves(history, save_path="learning_curves_gat.png"):
    ...
    ax1.set_title("Loss vs. Epochs (GAT)", fontsize=14, fontweight="bold")
    ...
    ax2.set_title("Validation Metrics vs. Epochs (GAT)", fontsize=14, fontweight="bold")
...
def plot_tsne_embeddings(emb_before, emb_after,
                         data=None,
                         save_path="tsne_clusters_gat.png"):
    """
    ...
    plt.suptitle(
        "t-SNE Visualization of GAT Node Embeddings\n"
...
if __name__ == "__main__":
    ...
    HIDDEN_CHANNELS = 128
    OUT_CHANNELS    = 64
    LEARNING_RATE   = 0.005  # Slightly lower LR for attention stability
    EPOCHS          = 100
    DROPOUT         = 0.5   # ICLR 2017 GCN recommendation
    WEIGHT_DECAY    = 5e-4  # L2 regularisation in Adam
    PATIENCE        = 10    # Early stopping patience (epochs)
    HEADS           = 4      # Number of attention heads in Layer 1
    ...
    print("=" * 60)
    print("  Step 3 — Model architecture (GAT — Graph Attention Network)")
    print("=" * 60)
    print(model)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_auc > best_val_auc:
            ...
            torch.save(model.state_dict(), os.path.join(OUT_DIR, "best_model_gat.pt"))
            marker = "  ◀ best"
        else:
            patience_counter += 1
    ...
    plot_learning_curves(history, save_path=os.path.join(OUT_DIR, "learning_curves_gat.png"))
    plot_tsne_embeddings(emb_epoch1, emb_final, data=data, save_path=os.path.join(OUT_DIR, "tsne_clusters_gat.png"))
    ...
    model.load_state_dict(torch.load(os.path.join(OUT_DIR, "best_model_gat.pt"), weights_only=True))
